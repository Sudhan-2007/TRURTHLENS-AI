...
- `pytest -q` (from `backend\`) — app suite (229 passing; asyncio auto mode;
  pytest-asyncio session-scoped loops). Needs a running MongoDB
  (localhost:27017 or the compose service). Coverage: 99% (module `app`);
...
- `npm run lint` — oxlint.
- `npm test` — vitest (107 tests; coverage gate: 93% lines, CI enforces
  >= 80% lines / >= 75% branches via `npm run test:coverage`).
