...
passing**; AI engine: **14 unit tests**; frontend: **20 component tests**.
...
| Frontend | `src/components/__tests__/*.test.jsx` (Vitest + Testing Library) | `TrustScore` empty/scored/interaction states, `StatsCard` rendering, `HistoryTable` empty/badge/link/delete behavior, `EvidenceList` empty/badge/similarity/source-link states, `ProtectedRoute` loading/redirect/authenticated states |
